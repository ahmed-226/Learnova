import React from 'react';

const RecentActivityCard = ({ activity }) => {
  // Function to render the appropriate icon based on activity type
  const renderIcon = () => {
    switch (activity.icon) {
      case 'book-open':
        return (
          <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
          </svg>
        );
      case 'check-circle':
        return (
          <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
        );
      case 'clipboard':
        return (
          <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
          </svg>
        );
      default:
        return (
          <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
        );
    }
  };

  // Function to render activity message based on type
  const renderActivityMessage = () => {
    switch (activity.type) {
      case 'course-progress':
        return (
          <>
            You made progress in <span className="font-medium">{activity.courseName}</span>
            <div className="mt-1 w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5">
              <div className="bg-primary-600 h-1.5 rounded-full" style={{ width: `${activity.progress}%` }}></div>
            </div>
          </>
        );
      case 'quiz-completed':
        return (
          <>
            You scored <span className="font-medium">{activity.score}%</span> in <span className="font-medium">{activity.courseName}</span> quiz
          </>
        );
      case 'assignment-submitted':
        return (
          <>
            You submitted an assignment for <span className="font-medium">{activity.courseName}</span>
          </>
        );
      default:
        return (
          <>
            Activity related to <span className="font-medium">{activity.courseName}</span>
          </>
        );
    }
  };

  return (
    <div className="flex items-start p-4 rounded-lg border border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
      <div className="bg-primary-100 dark:bg-primary-900/30 p-2 rounded-md text-primary-700 dark:text-primary-400 mr-4 flex-shrink-0">
        {renderIcon()}
      </div>
      <div className="flex-grow">
        <p className="text-gray-800 dark:text-gray-200">
          {renderActivityMessage()}
        </p>
        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{activity.date}</p>
      </div>
    </div>
  );
};

export default RecentActivityCard;